...
export const SensorCards = ({ onUpdate }: SensorCardsProps) => {
  ...
  // Update stats when new value arrives
  ...
  const handleMessage = useCallback((topic: string, message: Buffer) => {
    ...
    // Usar el tópico completo para mayor precisión
    switch(topic) {
  ...
  useEffect(() => {
    ...
    if (!globalClient) {
      globalClient = mqtt.connect("ws://broker.hivemq.com:8000/mqtt", {
        protocolVersion: 5,
      ...
      globalClient.on("connect", () => {
        setIsConnected(true);
        globalClient?.subscribe("karensensors/#", { qos: 0 }, (err) => {
          if (!err) console.log("Suscripción exitosa a todos los sensores");
        });
      });

      globalClient.on("message", handleMessage);
      globalClient.on("error", (err) => console.error("Error MQTT:", err));
      globalClient.on("reconnect", () => setIsConnected(false));
      globalClient.on("offline", () => setIsConnected(false));
    } else if (globalClient.connected) {
      setIsConnected(true);
      // Reagregar handler en caso de recarga
      globalClient.on("message", handleMessage);
    }

    // Limpieza: solo remover handlers
    return () => {
      if (globalClient) {
        globalClient.off("message", handleMessage);
      }
    };
  }, [handleMessage]);
